...
class BSTRAND {
    ...
    void insert_random(Node*&root, V value, K key){

        srand(time(0));
        int  x= rand() % size(root)+1;

        if (x==0) {
            insert_at_root(root, value, key);
        }

        else
            
            insert_at_leaf(root, value, key);
...
public:
    ...
    void insert (K key, V value){
        ...
        this->insert_random(this->root, value, key);
